...
void init()
{
	...
	{
		scanf("%lld", &h[i]);
		s[i] = (long long)s[i-1] + h[i];
	}
	build(1, 0, n);
}

void flagDown(ll now)
{
	if(nd[now].flag)
	{
		nd[now].num += nd[now].flag;
		nd[now*2].flag += nd[now].flag;
		nd[now*2+1].flag += nd[now].flag;
		nd[now].flag = 0;
	}
}
...
void solve()
{
	...
	for(ll i=1; i<=n; ++i) if(h[i] > m) { puts("-1"); return; }
