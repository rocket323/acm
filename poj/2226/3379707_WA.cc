...
node A[2551],B[2551];
int n,m,tn,ttmm,link[2551]={0};
bool w[2551][2551]={0},g[51][51],used[2551];
...
*/	for(i=1;i<=n;i++){
		j=1;
		if(j==m){ tn++; A[tn].x=i; A[tn].y=1; A[tn].len=1; }
		while(j<m){
			while(g[i][j]==0) j++;
			if(j>m) break;
			tn++; len=0;
	...
	for(i=1;i<=m;i++){
		j=1;
		if(j==n){ ttmm++; B[ttmm].x=1; B[ttmm].y=i; B[ttmm].len=1; }
		while(j<n){
			while(g[j][i]==0) j++;
			if(j>n) break;
			ttmm++; len=0;
