...
void dfs(int now, int start, int &Max, int last, int &edg){
	...
	for(i=0;i<N-1;i++){
		...
		if(Mst[i].start==now&&Mst[i].end!=last){
			dfs(Mst[i].end, start, Max, now, edg);
			if(Find){
				if(Max<Mst[i].weight&&Mst[i].end!=0){
					Max=Mst[i].weight;
					edg=i;
				}
				return;
		...
		else if(Mst[i].start!=last&&Mst[i].end==now){
...
void process(){
	int i,j,k, Min, rec, MaxJ, MaxE, MaxEdge, Max;
	ans=prim( );
	Min=MaxL+1;
