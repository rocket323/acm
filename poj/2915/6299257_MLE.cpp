...
short n, m, f[MaxL][MaxL][22], pre[MaxL], low[MaxL][MaxL][22], g[MaxL][MaxL][22][22];
			...
			if(pre[j] != -1)
			{
				for(short k=1; k<m; ++k)
				{
					for(short c=max(1, m-k); c<m; ++c)
					{
						...
						//if(i==1 && j==5 && k==2 && c==1) printf("here %d %d\n", min(pre[j], s[k][c][i+1][j]), e);
						for(short p=min(pre[j], s[k][c][i+1][j]); p >= e; p=pre[p])
						{
							//if(i==1 && j==5 && k==2 && c==1) printf("here %d\n", p);
							if((tmp = f[i][p][k] + f[p+1][j][c]) <= g[k][c][i][j])
							{
								g[k][c][i][j] = tmp;
