...
adj[maxl + 10], memo[maxl * 10];

int d[maxl + 10], mset, n, a, b, c;
int Q[maxl + 10], head, tail, in[maxl + 10];

void addEdge(int a, int b, int c)
{
	//if(b < 20) printf("e %d %d %d\n", a, b, c);
	Edge *ptr = &memo[mset++];
	ptr->idx = b;
	ptr->len = c;
	ptr->next = adj[a].next;
...
void init()
{
	...
	for(int i=0; i<maxl; ++i) addEdge(i + 1, i, 0), addEdge(i, i + 1, 1);
