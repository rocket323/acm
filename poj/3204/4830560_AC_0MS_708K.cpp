...
adj2[MaxL], adj[MaxL], memo[200000];
...
void dfs(int now)
{
    ...
    while(ptr)
    {
        if(ptr->flag&&ptr->f>0&&!vis[ptr->idx]) dfs(ptr->idx);
        ptr = ptr->next;
    }
}
...
int cnt()
{
    ...
    for(int i=0; i<N; ++i)
    {
        ...
        while(ptr)
        {
            if(ptr->flag&&ptr->f==0&&vis[ptr->idx]==2) ans++;
            ptr = ptr->next;
        }
    }
    return ans;
...
void makeGraph()
{
    for(int i=0; i<N; ++i) adj2[i].next = NULL;
    for(int i=0; i<N; ++i)
    {
        Edge * ptr = adj[i].next;
        while(ptr)
        {
            if(ptr->f>0) addEdge2(ptr->idx, i);
            ptr = ptr->next;
        }
    }
}
