/*
	poj1815
	用最小割最大流求 最少删去多少个点使得图中两个点s，t不连通

	由于要保证方案的字典序最小，故从小到大枚举，如果删去后使得当前最大流减少，则删去，否则不删
*/
...
void build_Gra()
{
	...
	for(int j=1; j<=n; ++j) if(!del[j] && g[s][j]) addEdge(s, j, INF);
	for(int j=1; j<=n; ++j) if(!del[j] && g[t][j]) addEdge(j+n, t, INF);
}
