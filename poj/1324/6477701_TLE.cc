...
void solve()
{
	...
	pq.push(node(start, get_cost(sr[0], sc[0])));
	mp.insert(make_pair(start, get_cost(sr[0], sc[0])));
	...
	while(!pq.empty())
	{
		...
		int base = now.cost - get_cost(sr[0], sc[0]);
		...
		for(int k=0; k<4; ++k)
		{
			...
			int _cost = base + 1 + get_cost(tr[0], tc[0]);
