...
void process(){
	...
	for(i=1; i<N; ++i) change(1, s[B[i]], t[B[i]], C[i]);
	...
	for(i=1; i<=Q; ++i){
		...
		if(k==0){
			...
			d1=getTree(1, s[pos], t[pos]) / (t[pos]-s[pos]+1);
			d2=getTree(1, s[a], t[a]) / (t[a]-s[a]+1);
			d3=getTree(1, s[b], t[b]) / (t[b]-s[b]+1);
			printf("%d\n", d1+d2-2*d3);
			pos=a;
		}
		else{
