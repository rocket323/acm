...
Edge adj[MaxN], memo[2*MaxN];

int N, Q, pos, p[MaxN], s[MaxN], t[MaxN], id, mset;
int A[MaxN], B[MaxN], C[MaxN];
int L[MaxN], f[MaxN][17], rt;
...
int getTree(int now, int l, int r){
	...
	if(l<=mid) lv=getTree((now<<1), l, r);
	if(r>mid) rv=getTree((now<<1)+1, l, r);
	return lv+rv;
}
...
void change(int now, int l, int r, int add){
	...
	int mid=(nd[now].l+nd[now].r)>>1;
	if(l<=mid) change((now<<1), l, r, add);
	if(r>mid) change((now<<1)+1, l, r, add);
}
