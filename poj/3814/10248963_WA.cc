...
bool check(int cover) {
	...
	for(int i=0; i<n; ++i) {
		...
		for(int j=0; j<k[i]; ++j) {
			if(a[i][j] == b[i][j] && sa[i][j] != sb[i][j]) {
				for(int x=0; x<48; ++x) addEdge(x + 1, i + 48 + 1, 1);
			}
			else if(a[i][j] < b[i][j]) {
				for(int x=a[i][j]; x<b[i][j]; ++x) addEdge(x + 1, i + 48 + 1, 1);
			}
			else {
				for(int x=a[i][j]; x<48; ++x) addEdge(x + 1, i + 48 + 1, 1);
				for(int x=0; x<b[i][j]; ++x) addEdge(x + 1, i + 48 + 1, 1);
			}
		}
	}
	int flow = Dinic(s, t);
...
int contain(int a1, int b1, int a2, int b2) {
	if(a1 <= a2 && b1 >= b2) return 1;
	if(a2 <= a1 && b2 >= b1) return 1;
	return 0;
}
int away(int a1, int b1, int a2, int b2) {
	if(a1 > b2) return 1;
	if(b1 < a2) return 1;
	if(a2 > b1) return 1;
	if(b2 < a1) return 1;
...
int across(int a1, int b1, int a2, int b2) {
	if(a1 <= a2 && a2 <= b1) return 1;
	if(a1 <= b2 && b2 <= b1) return 1;
	return 0;
}
...
int main() {
	while(scanf("%d", &n), n) {
		...
		for(int i=0; i<n; ++i) {
			for(int j=0; j<k[i]; ++j) {
				for(int x=j+1; x<k[i]; ++x) {
					if(!contain(sa[i][j], sb[i][j], sa[i][x], sb[i][x]) && away(sa[i][j], sb[i][j], sa[i][x], sb[i][x])) continue;
					merge(sa[i][j], sb[i][j], sa[i][x], sb[i][x]);
					sa[i][x] = sa[i][k[i]-1];
					sb[i][x] = sb[i][k[i]-1];
					k[i]--;
					--x;
